<script>
    // Datos de ejemplo para las esculturas
    let esculturas = [
      { id: 1, nombre: "El Pensador", fechaCreacion: "1880-08-16" },
      { id: 2, nombre: "David", fechaCreacion: "1504-09-08" },
      { id: 3, nombre: "La Victoria Alada", fechaCreacion: "190 BC" },
    ];
    ...
    const eliminarEscultura = (id) => {
      console.log(`Eliminar escultura con ID: ${id}`);
  ...
  <div class="container">
    ...
    <table>
      ...
      <tbody>
        {#each esculturas as { id, nombre, fechaCreacion }}
          <tr>
            <td>{nombre}</td>
            <td>{fechaCreacion}</td>
            <td class="acciones">
              <button class="btn-editar" on:click={() => editarEscultura(id)}>✏️</button>
